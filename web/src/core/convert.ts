...
import { ConversionError, ConversionResult, ConvertOptions, SourceFile } from './types.js';
...
export type FormatId =
  | 'docx' | 'xlsx' | 'pptx' | 'odf' | 'epub' | 'pdf' | 'rtf' | 'html' | 'csv'
  | 'eml' | 'image' | 'text';
...
export function detectFormat(file: SourceFile): FormatInfo {
  ...
  if (ZipArchive.isZip(bytes)) return detectZip(bytes, extension);

  if (startsWith(bytes, [0xd0, 0xcf, 0x11, 0xe0])) {
    throw new ConversionError(
      'This is a legacy binary Office file (.doc/.xls/.ppt). Open it in Word, Excel or PowerPoint and save as .docx, .xlsx or .pptx, then convert that.'
    );
  }
  ...
  if (!looksTextual(bytes)) {
    throw new ConversionError(
      `“${file.name}” does not look like a document this converter can read. Supported formats include PDF, Word, Excel, PowerPoint, OpenDocument, EPUB, RTF, HTML, email, CSV and plain text.`
    );
  }
  ...
  if (extension === 'svg') return { id: 'text', label: 'SVG source' };

  return { id: 'text', label: textLabel(extension) };
}

function detectZip(bytes: Uint8Array, extension: string): FormatInfo {
  let zip: ZipArchive;
  try {
    zip = ZipArchive.open(bytes);
  } catch (err) {
    throw new ConversionError(`This ZIP-based file could not be opened: ${(err as Error).message}`);
  }

  if (zip.has('word/document.xml')) return { id: 'docx', label: 'Word (.docx)' };
  if (zip.has('xl/workbook.xml')) return { id: 'xlsx', label: 'Excel (.xlsx)' };
  if (zip.has('ppt/presentation.xml')) return { id: 'pptx', label: 'PowerPoint (.pptx)' };
  ...
  throw new ConversionError(
    `“.${extension}” is a ZIP archive, but not a document format this converter understands. Unzip it first and convert the files inside.`
  );
}

function textLabel(extension: string): string {
  switch (extension) {
    case 'md':
    case 'markdown':
      return 'Markdown';
...
export function convertFile(file: SourceFile, options: ConvertOptions): ConversionResult {
...
function runConverter(id: FormatId, file: SourceFile, options: ConvertOptions): RawOutput {
  switch (id) {
    case 'docx':
...
function convertHtmlFile(file: SourceFile, options: ConvertOptions): RawOutput {
  ...
  const markdown = htmlStringToMarkdown(text, {
    ...
    resolveUrl: (url, kind) => {
      if (kind === 'image' && options.imageMode === 'skip') return null;
      return url;
    },
  });

  const meta: Record<string, string> = { encoding, ...htmlMeta(root) };
...
function withoutTitle(meta: Record<string, string>): Record<string, string> {
  const copy = { ...meta };
  delete copy.title;
  return copy;
}
...
export function outputNameFor(name: string): string {
