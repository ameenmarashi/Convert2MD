...
export interface XElement {
  type: 'element';
  /** Raw qualified name as written, e.g. `w:p`. */
  name: string;
  /** Name without its namespace prefix, lower-cased for HTML. */
  local: string;
...
export function decodeEntities(input: string): string {
  ...
  return input.replace(/&(#x[0-9a-fA-F]+|#\d+|[a-zA-Z][a-zA-Z0-9]{1,31});/g, (match, body: string) => {
    if (body[0] === '#') {
...
function makeElement(name: string, attrs: Record<string, string>, html: boolean): XElement {
...
function appendText(parent: XElement, value: string): void {
  if (!value) return;
...
function parse(src: string, options: ParseOptions): XElement {
  ...
  while (i < len) {
    ...
    if (lt > i) appendText(top(), decodeEntities(src.slice(i, lt)));
    ...
    if (src.startsWith('</', lt)) {
      ...
      if (options.html) name = name.toLowerCase();
      closeTag(name);
      i = end + 1;
      continue;
    }
    ...
    if (options.html) applyImplicitClose(stack, element.local);
...
function applyImplicitClose(stack: XElement[], tag: string): void {
  ...
  if (current.local === 'p' && CLOSES_P.has(tag)) return closeCurrent();
  if (current.local === 'li' && tag === 'li') return closeCurrent();
  if ((current.local === 'dt' || current.local === 'dd') && (tag === 'dt' || tag === 'dd')) {
    return closeCurrent();
  }
  if ((current.local === 'td' || current.local === 'th') && ['td', 'th', 'tr'].includes(tag)) {
    closeCurrent();
    if (tag === 'tr' && stack[stack.length - 1]?.local === 'tr') stack.pop();
    return;
  }
  if (current.local === 'tr' && tag === 'tr') return closeCurrent();
  if (current.local === 'option' && (tag === 'option' || tag === 'optgroup')) return closeCurrent();
  if (['thead', 'tbody', 'tfoot'].includes(current.local) && ['thead', 'tbody', 'tfoot'].includes(tag)) {
    return closeCurrent();
  }
  if (current.local === 'head' && tag === 'body') return closeCurrent();
}

function findRawTextEnd(src: string, from: number, tag: string): { textEnd: number; next: number } {
  const lower = src.toLowerCase();
  const close = lower.indexOf(`</${tag}`, from);
  if (close === -1) return { textEnd: src.length, next: src.length };
...
function readTag(src: string, start: number, html: boolean): TagInfo | null {
  let i = start + 1;
  ...
  while (i < src.length) {
    ...
    if (html) attrName = attrName.toLowerCase();
...
export function parseHtml(src: string): XElement {
...
function matches(el: XElement, query: string): boolean {
  return query.includes(':') ? el.name === query : el.local === query;
}

export function children(el: XElement, query?: string): XElement[] {
  const out: XElement[] = [];
  for (const node of el.children) {
    if (node.type === 'element' && (!query || matches(node, query))) out.push(node);
  }
  return out;
}

export function child(el: XElement, query: string): XElement | null {
  for (const node of el.children) {
    if (node.type === 'element' && matches(node, query)) return node;
  }
  return null;
}

/** Depth-first descendants matching `query`, document order. */
export function descendants(el: XElement, query: string): XElement[] {
  const out: XElement[] = [];
  const walk = (node: XElement): void => {
    for (const kid of node.children) {
      if (kid.type !== 'element') continue;
      if (matches(kid, query)) out.push(kid);
      walk(kid);
    }
  };
  walk(el);
...
export function firstDescendant(el: XElement, query: string): XElement | null {
  for (const kid of el.children) {
    if (kid.type !== 'element') continue;
    if (matches(kid, query)) return kid;
...
export function attr(el: XElement, query: string): string | null {
  const direct = el.attrs[query];
  if (direct !== undefined) return direct;
  if (query.includes(':')) return null;
  for (const [key, value] of Object.entries(el.attrs)) {
    const colon = key.indexOf(':');
    if (colon >= 0 && key.slice(colon + 1) === query) return value;
  }
  return null;
}
...
export function elementPath(el: XElement): string {
  const parts: string[] = [];
  let cur: XElement | null = el;
