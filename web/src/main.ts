...
const dom = {
  ...
  mdInput: byId<HTMLInputElement>('md-input'),
  openMdButton: byId<HTMLButtonElement>('open-md-button'),
  newButton: byId<HTMLButtonElement>('new-button'),
  library: byId<HTMLElement>('library'),
  libraryList: byId<HTMLUListElement>('library-list'),
  libraryNote: byId<HTMLElement>('library-note'),
...
function openFromLibrary(id: string): void {
...
function isMarkdownFile(file: File): boolean {
  ...
  if (MARKDOWN_EXTENSIONS.some((extension) => name.endsWith(extension))) return true;
...
async function addFiles(files: File[]): Promise<void> {
  ...
  if (markdown.length > 0) {
    ...
    if (markdown.length > 1) toast(`Opened ${markdown[0].name} — Markdown files are read one at a time.`);
...
async function saveBlob(blob: Blob, filename: string): Promise<void> {
  if (await trySaveWithPicker(blob, filename)) return;
 ...
 */
async function trySaveWithPicker(blob: Blob, filename: string): Promise<boolean> {
