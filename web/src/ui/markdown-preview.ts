  ...
  while (index < lines.length) {
    ...
    if (listMarker(line)) {
      ...
      while (index < lines.length && (lines[index].trim() || nextIsListContinuation(lines, index))) {
        if (!lines[index].trim() && !nextIsListContinuation(lines, index)) break;
...
function frontMatterBlock(yaml: string): HTMLElement {
...
function codeBlockElement(body: string, language: string): HTMLElement {
  const pre = document.createElement('pre');
  const code = document.createElement('code');
...
function tableElement(rows: string[]): HTMLElement {
...
function applyAlignment(cell: HTMLElement, align: string | undefined): void {
...
function listMarker(line: string): { ordered: boolean; indent: number; content: string } | null {
  const bullet = line.match(/^(\s*)[-*+]\s+(.*)$/);
  if (bullet) return { ordered: false, indent: bullet[1].length, content: bullet[2] };
  const ordered = line.match(/^(\s*)\d{1,9}[.)]\s+(.*)$/);
  if (ordered) return { ordered: true, indent: ordered[1].length, content: ordered[2] };
  return null;
}

function nextIsListContinuation(lines: string[], index: number): boolean {
  const next = lines[index + 1];
  return Boolean(next && (listMarker(next) || /^\s{2,}\S/.test(next)));
}

function listElement(block: string[]): HTMLElement {
...
function buildList(items: ListItem[], depth: number): HTMLElement {
  ...
  while (index < items.length) {
    ...
    if (nestedItems.length > 0 && depth < 6) li.append(buildList(nestedItems, depth + 1));
...
export function inline(text: string): Node[] {
  ...
  while (rest.length > 0) {
    ...
    if (match.index > 0) nodes.push(document.createTextNode(unescapeMarkdown(rest.slice(0, match.index), false)));
    const token = match[0];
...
function inlineNode(token: string): Node {
  if (token.startsWith('![')) {
    const parsed = parseLink(token.slice(1));
    const img = document.createElement('img');
    if (SAFE_IMAGE.test(parsed.href)) img.src = parsed.href;
    img.alt = parsed.text;
  ...
  if (token.startsWith('[')) {
    ...
    if (SAFE_SCHEME.test(parsed.href)) anchor.href = parsed.href;
    anchor.rel = 'noopener noreferrer';
    anchor.target = '_blank';
    if (parsed.title) anchor.title = parsed.title;
  ...
  if (/^<br\s*\/?>$/.test(token)) return document.createElement('br');
...
function parseLink(token: string): { text: string; href: string; title: string } {
...
function findClosingBracket(token: string): number {
  let depth = 0;
...
function unescapeMarkdown(text: string, keepStructural: boolean): string {
  if (keepStructural) return text;
