...
export function initReader(handlers: {
  ...
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && isOpen()) closeReader();
  });

  // Android's back gesture should leave the document, not the app.
  window.addEventListener('popstate', () => {
    if (isOpen()) closeReader({ fromHistory: true });
  });
}
...
function buildContents(): void {
  if (!dom) return;
  const headings = [...dom.doc.querySelectorAll<HTMLElement>('h1, h2, h3')];
...
function stepSize(direction: number): void {
