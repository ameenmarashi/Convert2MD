...
export function convertCsv(file: SourceFile, _options: ConvertOptions): CsvOutput {
...
function inferAlignment(rows: string[][]): (('l' | 'c' | 'r') | null)[] {
  if (rows.length < 2) return [];
  const width = rows[0].length;
  const align: (('l' | 'c' | 'r') | null)[] = [];
  ...
  for (let c = 0; c < width; c++) {
    ...
    for (let r = 1; r < rows.length; r++) {
      ...
      if (!cell) continue;
      filled++;
      if (/^[-+]?[\d.,]+%?$/.test(cell) && /\d/.test(cell)) numeric++;
    }
