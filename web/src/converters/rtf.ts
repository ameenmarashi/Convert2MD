...
export interface RtfOutput {
  markdown: string;
  meta: Record<string, string>;
  warnings: string[];
}

export function convertRtf(file: SourceFile, options: ConvertOptions): RtfOutput {
...
class RtfParser {
  private pos = 0;
  private readonly stack: RtfState[] = [];
  private state: RtfState = {
    bold: false,
    italic: false,
    strike: false,
    hidden: false,
  ...
  private current: Paragraph = newParagraph();
  private pendingCells: string[] = [];
  private tableRows: string[][] = [];
  private readonly meta: Record<string, string> = {};
  private readonly warnings: string[] = [];
  private linkTarget: string | null = null;
  private fieldInstruction: string | null = null;
  private skipCharacters = 0;

  constructor(private readonly src: string, private readonly options: ConvertOptions) {}

  parse(): RtfOutput {
    if (!this.src.startsWith('{\\rtf')) this.warnings.push('File does not start with an RTF header; parsed anyway.');
    ...
    while (this.pos < this.src.length) {
      ...
      if (ch === '}') {
        ...
        if (restored) {
          if (this.state.destination === 'fldrslt' && restored.destination !== 'fldrslt') this.linkTarget = null;
          this.state = restored;
        }
        continue;
      }
  ...
  private readControl(): void {
    ...
    if (ch === "'") {
      ...
      if (Number.isFinite(code)) {
        if (this.skipCharacters > 0) this.skipCharacters--;
        else this.pushText(CP1252_HIGH[code] ?? String.fromCharCode(code));
      }
      return;
    }
    ...
    if (!/[a-zA-Z]/.test(ch)) {
      ...
      if (ch === '\\' || ch === '{' || ch === '}') {
        ...
        if (mapped !== undefined) this.pushText(mapped);
      }
      return;
    }
  ...
  private markIgnorableDestination(): void {
  ...
  private applyControlWord(word: string, parameter: number | null): void {
    ...
    switch (word) {
      ...
      case 'pghsxn':
        return;
      default:
        if (CONTROL_SYMBOLS[word] !== undefined) this.pushText(CONTROL_SYMBOLS[word]);
        return;
    }
  }

  private readMetaGroup(key: string): void {
    const start = this.pos;
    let depth = 1;
    let text = '';
    while (this.pos < this.src.length && depth > 0) {
  ...
  private pushText(text: string): void {
    ...
    if (this.state.destination === 'fldinst') {
      ...
      if (match) this.linkTarget = match[1];
      return;
    }
  ...
  private endParagraphBreak(): void {
    this.endParagraph();
    this.current = newParagraph();
  }

  private endParagraph(): void {
    if (this.current.runs.length === 0) return;
...
function renderRuns(runs: Run[]): string {
  ...
  for (const run of runs) {
    ...
    if (run.href) core = mdLink(core, run.href);
...
function stripWrappingEmphasis(text: string): string {
  ...
  if (bold && !bold[1].includes('**')) return bold[1];
  const italic = text.match(/^\*(.+)\*$/s);
  if (italic && !italic[1].includes('*')) return italic[1];
  return text;
}

function medianFontSize(paragraphs: Paragraph[]): number {
