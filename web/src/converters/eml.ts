...
export function convertEml(file: SourceFile, options: ConvertOptions): EmlOutput {
  ...
  if (from) meta.author = from;
  if (date) meta.date = date;

  const summary: string[][] = [['Field', 'Value']];
  if (from) summary.push(['From', from]);
  if (to) summary.push(['To', to]);
  if (cc) summary.push(['Cc', cc]);
  if (date) summary.push(['Date', date]);
  if (summary.length > 1) writer.push(renderTable(summary));

  if (body.trim()) writer.push(body);
...
function collectParts(part: MimePart): { html: string; plain: string; attachments: string[] } {
  let html = '';
  let plain = '';
  const attachments: string[] = [];

  const walk = (node: MimePart): void => {
    ...
    if (isAttachment) {
      if (node.filename) attachments.push(node.filename);
      return;
    }
...
function parsePart(bytes: Uint8Array): MimePart {
  ...
  const filename =
    decodeHeaderValue(parameterOf(dispositionRaw, 'filename') || parameterOf(contentTypeRaw, 'name'));
  ...
  if (encoding === 'base64') body = decodeBase64(body);
...
function splitHeaders(bytes: Uint8Array): { headerText: string; body: Uint8Array } {
...
function parameterOf(header: string, name: string): string {
...
export function decodeHeaderValue(value: string): string {
  ...
  return value
    ...
    .replace(/=\?([^?]+)\?([BbQq])\?([^?]*)\?=/g, (_match, charset: string, kind: string, payload: string) => {
      try {
        const bytes =
          kind.toLowerCase() === 'b'
