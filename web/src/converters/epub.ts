...
export function convertEpub(file: SourceFile, options: ConvertOptions): EpubOutput {
  const zip = ZipArchive.open(file.bytes);
  const opfPath = findOpfPath(zip);
  const opfXml = zip.readText(opfPath);
  if (!opfXml) throw new ConversionError('Not an EPUB: the OPF package document is missing.');
  ...
  for (const id of spineIds) {
    ...
    const markdown = htmlToMarkdown(parseHtml(html), {
      ...
      resolveUrl: (url, kind) => {
        ...
        if (options.imageMode === 'reference') return basename(path);
...
function findOpfPath(zip: ZipArchive): string {
...
function readEpubMeta(opf: ReturnType<typeof parseXml>): Record<string, string> {
