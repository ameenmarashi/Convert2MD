...
import { TextItem } from './content.js';
...
function mergeLine(items: TextItem[]): Line {
  ...
  for (const item of ordered) {
    ...
    if (text && gap > reference * 0.22 && !text.endsWith(' ') && !item.text.startsWith(' ')) text += ' ';
    text += item.text;
...
export function stripRunningHeadersAndFooters(pages: Line[][]): void {
...
export function pageToMarkdown(lines: Line[], options: PdfLayoutOptions, bodySize: number, sizeLevels: number[]): string {
  ...
  for (const block of blocks) {
    ...
    if (out) out += previousKind === 'list' && block.kind === 'list' ? '\n' : '\n\n';
    out += text;
...
function renderBlock(block: Block, options: PdfLayoutOptions): string {
...
function matchListItem(text: string): { ordered: boolean; marker: string; text: string } | null {
  const bullet = text.match(BULLET_PATTERN);
  if (bullet && bullet[2].trim()) return { ordered: false, marker: bullet[1], text: bullet[2].trim() };
...
function headingLevelFor(line: Line, bodySize: number, sizeLevels: number[]): number {
...
export function bodyFontSize(pages: Line[][]): number {
...
export function headingSizeLevels(pages: Line[][], bodySize: number): number[] {
