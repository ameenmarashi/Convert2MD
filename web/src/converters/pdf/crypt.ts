...
import { PdfDict, PdfName, PdfObject, PdfString } from './lexer.js';
...
export type CryptMethod = 'rc4' | 'aes' | 'identity';
...
export class Decryptor {
  private constructor(
    ...
    private readonly method: CryptMethod,
    private readonly revision: number
  ) {}

  static create(encrypt: PdfDict, firstId: Uint8Array, resolve: (o: PdfObject) => PdfObject): Decryptor {
    ...
    if (v >= 4) {
      ...
      if (cfName === 'Identity') {
        ...
        const entry = resolve((cf as PdfDict).get(cfName) ?? null);
        const cfm = entry instanceof Map ? nameOf(resolve((entry as PdfDict).get('CFM') ?? null)) : null;
        const cfLength = entry instanceof Map ? numberOf(resolve((entry as PdfDict).get('Length') ?? null), 0) : 0;
        if (cfm === 'AESV2') {
          method = 'aes';
          keyLengthBytes = 16;
        } else if (cfm === 'AESV3') {
          ...
          method = 'rc4';
          if (cfLength > 0) keyLengthBytes = cfLength > 40 ? Math.floor(cfLength / 8) : cfLength;
        }
      }
    }
  ...
  decrypt(data: Uint8Array, num: number, gen: number): Uint8Array {
    if (this.method === 'identity') return data;
    if (this.revision >= 5) return aesCbcDecrypt(this.key, data);
...
function deriveLegacyKey(
  ...
  parts.push(...firstId);
  if (revision >= 4 && !encryptMetadata) parts.push(0xff, 0xff, 0xff, 0xff);
...
function deriveR5Key(u: Uint8Array, ue: PdfObject): Uint8Array {
  if (u.length < 48) throw new DecryptionError('Encrypted PDF is missing its /U entry.');
  const keySalt = u.subarray(40, 48);
  const intermediate = sha256(concat(new Uint8Array(0), keySalt));
  const ueBytes = stringBytes(ue);
  if (ueBytes.length < 32) throw new DecryptionError('Encrypted PDF is missing its /UE entry.');
...
function stringBytes(value: PdfObject): Uint8Array {
...
function padMessage(input: Uint8Array, littleEndian: boolean): Uint8Array {
...
function expandKey(key: Uint8Array): Uint8Array[] {
  ...
  for (let round = 0; round <= nr; round++) {
    ...
    for (let col = 0; col < 4; col++) rk.set(w[round * 4 + col], col * 4);
    roundKeys.push(rk);
  }
  return roundKeys;
}

function decryptBlock(roundKeys: Uint8Array[], block: Uint8Array): Uint8Array {
...
function addRoundKey(state: Uint8Array, key: Uint8Array): void {
...
export function aesCbcNoPadDecrypt(key: Uint8Array, iv: Uint8Array, data: Uint8Array): Uint8Array {
  ...
  for (let i = 0; i < blocks; i++) {
    ...
    for (let j = 0; j < 16; j++) plain[j] ^= previous[j];
    out.set(plain, i * 16);
    previous = block;
  }
  return out;
