...
export class PdfDocument {
  ...
  private readonly offsets = new Map<number, ObjectSlot>();
  private readonly cache = new Map<number, PdfObject>();
  private readonly compressed = new Map<number, PdfObject>();
  private decryptor: Decryptor | null = null;
  private trailer: PdfDict = new Map();
  private objStmExpanded = false;
  ...
  info(): Record<string, string> {
    ...
    if (!(info instanceof Map)) return meta;

    const read = (key: string): string => {
      const value = this.resolve((info as PdfDict).get(key) ?? null);
    ...
    if (created) meta.created = formatPdfDate(created);
    const modified = read('ModDate');
    if (modified) meta.modified = formatPdfDate(modified);
    return meta;
  }

  resolve(value: PdfObject): PdfObject {
    let current = value;
    let guard = 0;
    while (current instanceof PdfRef && guard++ < 64) current = this.getObject(current.num, current.gen);
    return current;
  }

  dictGet(dict: PdfDict | null, ...keys: string[]): PdfObject {
    if (!dict) return null;
    for (const key of keys) {
      if (dict.has(key)) return this.resolve(dict.get(key) ?? null);
    }
    return null;
  }

  /** Fully decoded bytes for a stream object. */
  streamData(stream: PdfStreamObject, owner?: PdfRef): Uint8Array {
    let raw = stream.raw;
    if (this.decryptor && owner) {
      const type = this.dictGet(stream.dict, 'Type');
      const isXref = type instanceof PdfName && type.name === 'XRef';
      if (!isXref) raw = this.decryptor.decrypt(raw, owner.num, owner.gen);
    }
    try {
      return decodeStream(stream.dict, raw, (o) => this.resolve(o)).data;
    } catch (err) {
      this.warnings.push(`Could not decode a stream: ${(err as Error).message}`);
  ...
  private getObject(num: number, gen = 0): PdfObject {
    ...
    try {
      ...
      this.warnings.push(`Object ${num} could not be read: ${(err as Error).message}`);
      value = null;
    }
    if (value === null) {
      const fromStream = this.compressed.get(num);
      if (fromStream !== undefined) value = fromStream;
    }
    this.cache.set(num, value);
    return value;
  }

  private parseObjectAt(offset: number, expectedNum: number, gen: number): PdfObject {
    const lexer = new Lexer(this.bytes, offset);
    const num = lexer.next();
    lexer.next(); // generation
    const keyword = lexer.next();
    if (!(keyword instanceof PdfOperator) || keyword.op !== 'obj') return null;
    if (typeof num === 'number' && num !== expectedNum) return null;
  ...
  private decryptStrings(value: PdfObject, num: number, gen: number): PdfObject {
    if (!this.decryptor) return value;

    const walk = (node: PdfObject): PdfObject => {
      ...
      if (node instanceof Map) {
        ...
        for (const [key, entry] of node) out.set(key, walk(entry));
        return out;
      }
      return node;
    };
  ...
  private looksLikeEndstream(end: number): boolean {
    for (let i = end; i < Math.min(end + 4, this.bytes.length); i++) {
      if (isSpace(this.bytes[i])) continue;
  ...
  private readTrailer(): void {
    ...
    if (!merged.has('Root') || !merged.has('Encrypt')) {
      for (const num of this.offsets.keys()) {
        ...
        const dict = value instanceof PdfStreamObject ? value.dict : value instanceof Map ? (value as PdfDict) : null;
        if (!dict) continue;
        const type = dict.get('Type');
        if (type instanceof PdfName && type.name === 'XRef') {
          for (const [key, entry] of dict) if (!merged.has(key)) merged.set(key, entry);
        }
      }
    }
  ...
  private setUpDecryption(): void {
    ...
    const firstId = Array.isArray(ids) && ids[0] instanceof PdfString ? (ids[0] as PdfString).bytes : new Uint8Array(0);

    try {
      this.decryptor = Decryptor.create(dict as PdfDict, firstId, (o) => this.resolve(o));
      // Objects parsed before the handler existed hold undecrypted strings.
      this.cache.clear();
    } catch (err) {
      if (err instanceof DecryptionError) throw err;
      throw new DecryptionError(`This PDF is encrypted and could not be opened: ${(err as Error).message}`);
    }
  }

  private expandObjectStreams(): void {
    if (this.objStmExpanded) return;
    this.objStmExpanded = true;

    for (const num of [...this.offsets.keys()]) {
      const value = this.getObject(num);
      if (!(value instanceof PdfStreamObject)) continue;
      ...
      try {
        ...
        this.warnings.push(`Object stream ${num} could not be read: ${(err as Error).message}`);
      }
    }
  }

  private readObjectStream(stream: PdfStreamObject, owner: PdfRef): void {
    const data = this.streamData(stream, owner);
    const count = this.dictGet(stream.dict, 'N');
    const first = this.dictGet(stream.dict, 'First');
    if (typeof count !== 'number' || typeof first !== 'number') return;
    ...
    for (const entry of entries) {
      if (this.offsets.has(entry.num)) continue;
      const lexer = new Lexer(data, first + entry.offset);
      const value = lexer.next();
      if (value !== undefined && !(value instanceof PdfOperator)) this.compressed.set(entry.num, value);
    }
  }
  ...
  private buildPages(): void {
    ...
    const catalog = root instanceof Map ? (root as PdfDict) : this.findCatalog();
    const pagesRoot = catalog ? this.dictGet(catalog, 'Pages') : null;

    if (pagesRoot instanceof Map) {
      const visited = new Set<PdfDict>();
      this.walkPageTree(pagesRoot as PdfDict, {}, visited);
    }

    if (this.pages.length === 0) this.collectLoosePages();
  }

  private findCatalog(): PdfDict | null {
    for (const num of this.offsets.keys()) {
      const value = this.getObject(num);
      if (!(value instanceof Map)) continue;
      const type = (value as PdfDict).get('Type');
      if (type instanceof PdfName && type.name === 'Catalog') return value as PdfDict;
    }
    return null;
  }
  ...
  private walkPageTree(
    node: PdfDict,
    inherited: { resources?: PdfDict; mediaBox?: number[]; rotate?: number },
    visited: Set<PdfDict>
  ): void {
    if (visited.has(node) || this.pages.length > 5000) return;
    visited.add(node);
    ...
    const context = {
      resources: resourcesValue instanceof Map ? (resourcesValue as PdfDict) : inherited.resources,
      mediaBox: Array.isArray(mediaBoxValue)
        ? mediaBoxValue.map((v) => (typeof this.resolve(v) === 'number' ? (this.resolve(v) as number) : 0))
        : inherited.mediaBox,
    ...
    if (Array.isArray(kids)) {
      for (const kid of kids) {
        ...
        if (child instanceof Map) this.walkPageTree(child as PdfDict, context, visited);
      }
      return;
    }
  ...
  private collectLoosePages(): void {
    ...
    for (const num of nums) {
      ...
      if (type instanceof PdfName && type.name === 'Page') {
        ...
        this.pushPage(dict, {
          resources: resources instanceof Map ? (resources as PdfDict) : undefined,
        });
      }
    }
  }

  private pushPage(dict: PdfDict, context: { resources?: PdfDict; mediaBox?: number[]; rotate?: number }): void {
  ...
  pageContent(page: PdfPage): Uint8Array {
    ...
    const add = (value: PdfObject, ref?: PdfRef): void => {
      ...
      if (resolved instanceof PdfStreamObject) {
        ...
        for (const item of resolved) add(item);
      }
    };
    add(contents);
