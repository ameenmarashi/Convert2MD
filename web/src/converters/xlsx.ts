...
import { attr, child, children, descendants, parseXml, textOf, XElement } from '../core/xml.js';
...
export function convertXlsx(file: SourceFile, options: ConvertOptions): XlsxOutput {
  const zip = ZipArchive.open(file.bytes);
  const workbookXml = zip.readText('xl/workbook.xml');
  if (!workbookXml) throw new ConversionError('Not a workbook: xl/workbook.xml is missing.');
...
function siText(si: XElement): string {
...
function readDateStyles(zip: ZipArchive): Set<number> {
  ...
  for (const numFmt of descendants(root, 'numFmt')) {
    ...
    if (Number.isFinite(id) && /[ymdhs]/.test(stripped)) customDateFormats.add(id);
  }
  ...
  children(cellXfs, 'xf').forEach((xf, index) => {
    ...
    if (DATE_FORMAT_IDS.has(id) || customDateFormats.has(id)) styles.add(index);
  });
  return styles;
}
...
function readSheet(
  ...
  const trimmed = grid.map((row) => {
    ...
    while (copy.length < maxColumn) copy.push('');
    return copy;
  });
  while (trimmed.length > 0 && trimmed[trimmed.length - 1].every((c) => c.trim() === '')) trimmed.pop();
  while (trimmed.length > 0 && trimmed[0].every((c) => c.trim() === '')) trimmed.shift();

  // Drop trailing columns that are empty everywhere.
  let width = maxColumn;
  while (width > 0 && trimmed.every((row) => (row[width - 1] ?? '').trim() === '')) width--;
  return trimmed.map((row) => row.slice(0, width));
}

function readSheetHyperlinks(sheetRoot: XElement, zip: ZipArchive, sheetPath: string): Map<string, string> {
...
function cellValue(cell: XElement, sharedStrings: string[], dateStyles: Set<number>): string {
