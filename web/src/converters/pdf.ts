...
export function convertPdf(file: SourceFile, options: ConvertOptions): PdfOutput {
  let doc: PdfDocument;
  try {
    doc = PdfDocument.parse(file.bytes);
  } catch (err) {
    if (err instanceof DecryptionError) throw new ConversionError(err.message);
    throw new ConversionError(`This PDF could not be read: ${(err as Error).message}`);
  }

  if (doc.pageCount === 0) throw new ConversionError('This PDF has no pages that could be read.');
  ...
  for (const page of doc.pages) {
    try {
      ...
      warnings.push(`Page ${pages.length + 1} could not be read: ${(err as Error).message}`);
      pages.push([]);
    }
  }
  ...
  if (writer.isEmpty) {
    throw new ConversionError(
      'No text could be extracted from this PDF. It is most likely a scan of paper — run it through OCR first, then convert the result.'
    );
  }
  if (emptyPages > 0) {
    warnings.push(
      `${emptyPages} of ${doc.pageCount} page${doc.pageCount === 1 ? '' : 's'} held no extractable text (images or scans).`
    );
  }
