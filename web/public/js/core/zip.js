...
export class ZipArchive {
    ...
    namesUnder(prefix, extension) {
        const matches = this.names.filter((n) => n.startsWith(prefix) && (!extension || n.toLowerCase().endsWith(extension)));
    ...
    findEocd() {
        ...
        for (let i = 22; i <= max; i++) {
            ...
            if (at < 0)
                break;
            if (this.view.getUint32(at, true) === EOCD_SIG)
                return at;
        }
    ...
    readCentralDirectory() {
        ...
        for (let i = 0; i < count; i++) {
            if (pos + 46 > this.bytes.length)
                break;
            if (this.view.getUint32(pos, true) !== CENTRAL_SIG)
                break;
                ...
                while (ex + 4 <= extraEnd) {
                    ...
                    if (headerId === 0x0001) {
                        ...
                        if (localOffset === 0xffffffff)
                            localOffset = Number(this.view.getBigUint64(field, true));
                        break;
                    }
                    ex += 4 + size;
                }
