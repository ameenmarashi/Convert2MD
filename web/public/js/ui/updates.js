...
export function initUpdates(options) {
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState !== 'visible')
            return;
        if (Date.now() - lastCheck < FOREGROUND_CHECK_INTERVAL)
            return;
...
async function check({ quiet }) {
    ...
    try {
        ...
        if (!quiet && !waiting)
            report('You are on the latest version.');
    }
    catch {
        if (!quiet) {
 ...
 */
async function showBuild(version) {
    ...
    try {
        ...
        if (hash)
            dom.version.textContent = `v${version} · build ${hash}`;
    }
    catch {
