 ...
 */
import { attr, child, descendants, parseHtml, textOf } from '../core/xml.js';
import { codeBlock, collapseWhitespace, escapeInline, image as mdImage, link as mdLink, normalizeMarkdown, renderTable, } from '../core/md.js';
...
export function htmlStringToMarkdown(html, options) {
    return htmlToMarkdown(parseHtml(html), options);
}
export function htmlToMarkdown(root, options) {
    const ctx = { ...options, listStack: [], classStyles: collectClassStyles(root) };
...
function styleForElement(el, ctx) {
...
export function htmlTitle(root) {
...
function renderList(el, ctx) {
    ...
    for (const li of el.children) {
        ...
        if (!body && nested.length === 0 && !checkbox)
            continue;
...
function taskMarker(li) {
    const input = descendants(li, 'input')[0];
    if (!input)
        return '';
    if ((attr(input, 'type') ?? '').toLowerCase() !== 'checkbox')
        return '';
    return attr(input, 'checked') !== null ? '[x] ' : '[ ] ';
}
function renderHtmlTable(el, ctx) {
    const rows = [];
    let headerRows = 0;
    ...
    for (const tr of rowElements) {
        ...
        if (cells.length === 0)
            continue;
        if (isHeaderRow && rows.length === headerRows)
            headerRows++;
        rows.push(cells);
    }
    if (rows.length === 0)
...
function renderInlineChildren(el, ctx) {
    let out = '';
    for (const node of el.children)
...
function renderInline(node, ctx) {
    if (node.type === 'text')
        return escapeInline(collapseWhitespace(node.value.replace(/\n/g, ' ')));
...
function wrap(inner, marker) {
    ...
    if (core.startsWith(marker) && core.endsWith(marker))
        return inner;
...
function trimInline(text) {
    return text.replace(/^[ \t]+/gm, (m, offset) => (offset === 0 ? '' : m)).replace(/^\s+|\s+$/g, '');
}
/**
...
function escapeLeadingMarker(text) {
    return text.replace(/^(\s*)([-+>#]|\d+[.)])(\s)/, (_match, indent, marker, tail) => {
