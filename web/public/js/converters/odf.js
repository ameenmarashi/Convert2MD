...
export function convertOdf(file, options) {
    const zip = ZipArchive.open(file.bytes);
    const contentXml = zip.readText('content.xml');
    if (!contentXml)
        throw new ConversionError('Not an OpenDocument file: content.xml is missing.');
    ...
    if (stylesXml)
        reader.loadStyles(parseXml(stylesXml));
    const body = firstDescendant(content, 'office:body');
    if (!body)
        throw new ConversionError('OpenDocument file has no body.');
...
class OdfReader {
    ...
    loadStyles(root) {
        for (const style of descendants(root, 'style:style')) {
            ...
            if (parent)
                this.styleParents.set(name, parent);
        ...
        for (const listStyle of descendants(root, 'text:list-style')) {
            ...
            for (const level of listStyle.children) {
                if (level.type !== 'element')
                    continue;
                const lvl = Number(attr(level, 'text:level') ?? '1') || 1;
                if (level.local === 'list-level-style-number')
                    ordered.set(lvl - 1, true);
    ...
    styleFor(name) {
        const empty = { bold: false, italic: false, strike: false, mono: false };
        if (!name)
            return empty;
    ...
    /* --------------------------------------------------------------- text */
    renderTextBody(container, writer) {
        ...
        switch (el.local) {
            ...
            case 'forms':
                for (const kid of el.children) {
                    if (kid.type === 'element')
                        this.renderBlock(kid, writer, depth);
                }
                break;
            case 'soft-page-break':
                if (this.options.pageSeparators)
                    writer.rule();
                break;
            default:
                for (const kid of el.children) {
    ...
    renderTableElement(table) {
        ...
        const pushRow = (row, header) => {
            ...
            if (cells.length === 0)
                return;
            if (header && rows.length === headerRows)
                headerRows++;
            rows.push(cells);
        };
        for (const node of table.children) {
            ...
            if (node.local === 'table-header-rows') {
                for (const row of children(node, 'table:table-row'))
                    pushRow(row, true);
            }
        ...
        while (width > 0 && rows.every((row) => (row[width - 1] ?? '').trim() === ''))
            width--;
        ...
        if (headerRows === 0)
            trimmed.unshift(new Array(width).fill(''));
    ...
    /* ------------------------------------------------------- spreadsheets */
    renderSpreadsheet(body, writer) {
        const tables = children(body, 'table:table');
        for (const table of tables) {
            if (tables.length > 1)
                writer.heading(2, attr(table, 'table:name') ?? 'Sheet');
    ...
    /* ------------------------------------------------------ presentations */
    renderPresentation(body, writer) {
        const pages = children(body, 'draw:page');
        pages.forEach((page, index) => {
            if (index > 0 && this.options.pageSeparators)
                writer.rule();
            ...
            for (const frame of descendants(page, 'draw:frame')) {
                ...
                for (const para of descendants(frame, 'text:p')) {
                    ...
                    if (text)
                        lines.push(`${this.options.bullet} ${text}`);
                }
                if (lines.length > 0)
            ...
            if (notes && this.options.includeNotes) {
                ...
                if (text)
                    writer.quote(`**Speaker notes:** ${text}`);
            }
        });
    }
    /* ------------------------------------------------------------- inline */
    renderInline(el, style = this.styleFor(attr(el, 'text:style-name'))) {
        let out = '';
    ...
    renderInlineNode(node, style) {
        if (node.type === 'text')
            return applyStyle(escapeInline(node.value), style);
        switch (node.local) {
            case 's': {
            ...
            case 'frame': {
                ...
                if (image)
                    return this.renderImageElement(image, attr(node, 'draw:name') ?? '');
