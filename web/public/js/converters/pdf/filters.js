...
import { PdfName } from './lexer.js';
export function decodeStream(dict, raw, resolve) {
...
function toArray(value, resolve) {
...
export function runLengthDecode(data) {
    ...
    while (i < data.length) {
        ...
        if (length < 128) {
            ...
            const byte = data[i++] ?? 0;
            for (let j = 0; j < 257 - length; j++)
                out.push(byte);
        }
    }
    return Uint8Array.from(out);
}
export function lzwDecode(data, earlyChange = 1) {
    const out = [];
    const dictionary = [];
    const reset = () => {
        dictionary.length = 0;
