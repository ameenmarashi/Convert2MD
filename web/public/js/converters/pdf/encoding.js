...
const GLYPH_TO_UNICODE = (() => {
    const map = { ...EXTRA_GLYPHS };
    for (const [code, name] of Object.entries(ASCII_NAMES))
        map[name] = String.fromCharCode(Number(code));
    for (let c = 65; c <= 90; c++)
...
export function baseEncodingTable(name) {
    ...
    switch (name) {
        case 'WinAnsiEncoding':
            for (let code = 128; code <= 159; code++)
                table[code] = WIN_ANSI_HIGH[code] ?? null;
            for (let code = 160; code <= 255; code++)
                table[code] = String.fromCharCode(code);
            break;
        case 'MacRomanEncoding':
            for (let code = 128; code <= 255; code++)
                table[code] = MAC_ROMAN_HIGH[code - 128] ?? null;
            break;
