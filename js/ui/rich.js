...
export function serializeEditable(root) {
    ...
    const live = root.querySelectorAll('input[type="checkbox"]');
    const copies = clone.querySelectorAll('input[type="checkbox"]');
    copies.forEach((copy, index) => {
        if (live[index]?.checked)
 ...
 */
export function applyRichCommand(root, command) {
    if (!focusInside(root))
        return false;
    switch (command) {
        case 'bold':
...
function insertLink() {
    ...
    if (!address || address === 'https://')
        return false;
    const href = /^[a-z][\w+.-]*:/i.test(address) ? address : `https://${address}`;
    if (selection && !selection.isCollapsed && !looksLikeUrl)
