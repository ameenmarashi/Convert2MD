...
export function convertPptx(file, options) {
    ...
    slidePaths.forEach((slidePath, index) => {
        ...
        if (index > 0 && options.pageSeparators)
            writer.rule();
        ...
        if (options.includeNotes) {
            ...
            if (notes)
                writer.quote(`**Speaker notes:** ${notes}`);
        }
    });
    if (writer.isEmpty)
        writer.paragraph('_The presentation contains no text._');
...
function renderTextBody(shape, bulleted = true) {
...
function renderParagraphRuns(para) {
    let out = '';
    for (const node of para.children) {
        ...
        if (hlink) {
            ...
            if (href)
                core = mdLink(core, `#${href}`);
        }
        out += `${leading}${core}${trailing}`;
    }
    return out;
}
function renderPptxTable(tbl) {
    const rows = [];
...
function renderPicture(pic, zip, rels, options, warnings) {
...
function readNotes(zip, rels) {
    for (const rel of rels.values()) {
        ...
        for (const shape of descendants(root, 'p:sp')) {
            if (placeholderType(shape) === 'sldNum')
                continue;
            const text = descendants(shape, 'a:t').map((t) => textOf(t)).join('');
            if (text.trim())
                texts.push(collapseWhitespace(text).trim());
        }
        return texts.join(' ');
    }
    return '';
