...
export function convertImage(file, options) {
    const writer = new MarkdownWriter();
    const warnings = [];
    const meta = { size: formatBytes(file.bytes.length) };
    const alt = file.name.replace(/\.[^.]+$/, '');
    const dimensions = readDimensions(file);
    if (dimensions)
        meta.dimensions = `${dimensions.width}×${dimensions.height}`;
    let imageCount = 0;
    if (options.imageMode === 'skip') {
        ...
        if (file.bytes.length > options.maxEmbeddedImageBytes && options.imageMode === 'embed') {
            warnings.push(`The image is ${formatBytes(file.bytes.length)}, above the inline limit, so it is linked by file name instead.`);
        }
