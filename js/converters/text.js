...
export function convertText(file, options) {
...
export function plainTextToMarkdown(text, options) {
...
function isHeadingLike(line, next) {
...
function jsonToMarkdown(text, extension, warnings) {
    if (extension === 'jsonl' || extension === 'ndjson') {
        const rows = text
            ...
            .filter((value) => isFlatRecord(value));
    ...
    try {
        ...
        warnings.push(`The JSON could not be parsed (${err.message}); kept as a code block.`);
...
function subtitlesToMarkdown(text) {
    const writer = new MarkdownWriter();
